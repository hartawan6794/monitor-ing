            ...
            <form action="{{ route('authorized_server.update', $authorized_server->id) }}" method="POST">
                ...
                <div class='mb-4'>
                    ...
                    <input type='text' class='form-control @error('ip_address') is-invalid @enderror' id='form-ip_address'
                        name='ip_address' value='{{ old("ip_address", $authorized_server->ip_address) }}'
                ...
                <div class='mb-4'>
                    ...
                    <input type='text' class='form-control @error('server_name') is-invalid @enderror' id='form-server_name'
                        name='server_name' value='{{ old("server_name", $authorized_server->server_name) }}'
                ...
                <div class='mb-4'>
                    ...
                    <input type='text' class='form-control @error('username') is-invalid @enderror' id='form-username'
                        name='username' value='{{ old("username", $authorized_server->username) }}'
                ...
                <div class='mb-4'>
                    ...
                    <input type='text' class='form-control @error('password') is-invalid @enderror' id='form-password'
                        name='password' value='{{ old("password", $authorized_server->password) }}'
                ...
                <div class='mb-4'>
                    ...
                    <input type='text' class='form-control @error('port') is-invalid @enderror' id='form-port' name='port'
                        value='{{ old("port", $authorized_server->port) }}' placeholder='Masukkan port'>
                    @error('port')
                        <div class='invalid-feedback'>{{ $message }}</div>
                    @enderror
                </div>
                <div class='mb-4'>
                    <label for='form-is_active' class='form-label !text-[.875rem] text-black'>Active</label>
                    <select name="is_active" id="is_active" class="form-control @error('is_active') is-invalid @enderror">
                        <option value="1" {{ $authorized_server->is_active == 1 ? 'selected' : '' }}>Active</option>
                        <option value="0" {{ $authorized_server->is_active == 0 ? 'selected' : '' }}>Inactive</option>
                    </select>
                    @error('is_active')
                        <div class='invalid-feedback'>{{ $message }}</div>
                    @enderror
